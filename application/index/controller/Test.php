...
class Test
{
    public $channelToken = 'ayhmVC45BhiSSwDON+oWCWKD1n0jmL6scIkcccu1FAKXahYxdkUAIR3OOx9b24qR5txTMrb9mxduvSUEEBkOBENb7EaYhxUJ1NrPE/iqNrJicCm+Y/O31JJe4Ix0TM+apz1BkRIiIT9toCAE99h4UwdB04t89/1O/w1cDnyilFU=';
...
//        $db = json_decode(file_get_contents($dbFilePath), true);
        ...
        foreach ($obj['events'] as &$event) {
            ...
            if (sizeof($verification['data']) == 0) {
                ...
                if ($httpStatus == '401') {//保存留言
                    ...
                    try {
                        db('leave_msg')->insert($leaveMsg);
                    } catch (\Exception $e) {
                        json(['code' => -2, 'data' => '', 'msg' => '留言失败']);
                    }
                    $message = '今担当者が外しています。営業時間：9:00 ~ 17:00';
                    json(['code' => 0, 'data' => '', 'msg' => '留言成功']);
                }
            }
    ...
    public function getCustomer()
    {
        ...
        define("SQL_PASSWORD", "b0e07d83aac94b26");
    ...
    public function reply($UserName, $Message)
    {
        $UserName = str_replace(' ','',$UserName);
        file_put_contents('log.txt', print_r($UserName, true));
        ...
        $sql = "select distinct customer_id from v2_customer where customer_name=':customer_name' and customer_ip = 'line' ";
        $sql = str_replace(":customer_name", $UserName, $sql);
        $result = $mysql->query($sql);
        $row = mysqli_fetch_assoc($result);
        file_put_contents('send.txt', print_r($row, true));
        mysqli_close($mysql);

        $userid = $row['customer_id'];
        file_put_contents('loguserid.txt', print_r($userid, true));
